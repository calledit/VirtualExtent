...
static void MakeUnitCube(ID3D11Buffer** outVB, ID3D11Buffer** outIB) {
	...
	const V verts[] = {
		...
		{-0.5f,-0.5f,+0.5f,  0,-1,0}, {+0.5f,-0.5f,+0.5f, 0,-1,0},
		{+0.5f,-0.5f,-0.5f,  0,-1,0}, {-0.5f,-0.5f,-0.5f, 0,-1,0},
		// +Z (forward for our meshes will be -Z; +Z is “back”)
		{+0.5f,-0.5f,+0.5f,  0,0,+1}, {-0.5f,-0.5f,+0.5f, 0,0,+1},
		{-0.5f,+0.5f,+0.5f,  0,0,+1}, {+0.5f,+0.5f,+0.5f, 0,0,+1},
		// -Z (this face points “forward” for the ray)
		{-0.5f,-0.5f,-0.5f,  0,0,-1}, {+0.5f,-0.5f,-0.5f, 0,0,-1},
		{+0.5f,+0.5f,-0.5f,  0,0,-1}, {-0.5f,+0.5f,-0.5f, 0,0,-1},
	};
	const uint16_t inds[] = {
...
static void DrawOne(const XrCompositionLayerProjectionView& view, int handIdx, const XMFLOAT4& color) {
	...
	// --- Draw pointer ray (thin box from grip forward) ---
	{
		// Ray goes along -Z in model space; scale length in Z, keep thin XY
		float thickness = 0.005f; // 5 mm
		float length = 3.0f;   // 3 meters
		XMMATRIX world =
		...
		XMStoreFloat4x4(&cb.world, XMMatrixTranspose(world));
		XMStoreFloat4x4(&cb.viewproj, XMMatrixTranspose(viewM * proj));
		cb.color = XMFLOAT4(color.x, color.y, color.z, 0.6f); // slightly translucent (not blended here, but consistent)
